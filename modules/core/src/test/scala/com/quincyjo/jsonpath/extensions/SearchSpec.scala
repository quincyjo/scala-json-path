...
class SearchSpec
  ...
  "Search" should "be true if the string contains a match for the regex" in {
    val cases = Table(
      ("given string", "given regex", "is a match"),
      ("foo", "f", true),
      ("foobar", "bar$", true),
      ("foobar", "foo$", false),
      ("1974-05-01", "1974", true),
      ("1974-06-01", "1975", false)
    )

    forAll(cases) { case (givenString, givenRegex, expectedResult) =>
      Search(LiteralString(givenString), LiteralString(givenRegex))(
        JsonBeanEvaluator,
        JsonBean.Null,
        JsonBean.Null
  ...
  it should "be false if the regex is not valid" in {
    Search(LiteralString("foo"), LiteralString("[*"))(
      JsonBeanEvaluator,
      JsonBean.Null,
      JsonBean.Null
    ) should be(false)
