...
final case class JsonPath(
  ...
  /** Apply this JsonPath to a JSON, returning a list of JSON values matching this path within the given JSON.
    * @param root The JSON to match against.
  ...
  /** Returns true if this path is absolute, IE; if it has is rooted in the root document.
    * @return A boolean indicating if this path is absolute.
    */
  def isAbsolute: Boolean = root.contains(Root)

  /** Returns true if this path is dynamic, IE; if it is rooted in the current document.
    * @return A boolean indicating if this path is dynamic.
    */
  def isDynamic: Boolean = root.contains(Current)

  /** Returns true if this path is relative, IE; if it has not root.
    * @return A boolean indicating if this path is relative.
    */
  def isRelative: Boolean = root.isEmpty

  /** Appends the given [[JsonPathNode]] to this path.
    * @param that The [[JsonPathNode]] to append.
  ...
  /** Appends all of the given [[JsonPathNode]]s to this path.
    * @param that An iterable of the [[JsonPathNode]]s to append.
  ...
  /** Prepends the given [[JsonPathNode]] to this path.
    * @param that The [[JsonPathNode]] to prepend.
    * @return This path with the given node prepended.
    */
  ...
  /** Prepends all of the given [[JsonPathNode]]s to this path.
    * @param that An iterable of the [[JsonPathNode]]s to prepend.
    * @return This path with the given nodes prepended.
    */
  ...
  /** DSL for appending many [[JsonPathNode]]s to this path.
    * Alias for [[appendedAll(Iterable[JsonPathNode])]]
    * @param path An iterable of the [[JsonPathNode]]s to append.
    * @return This path with the given nodes appended.
    * @see [[appendedAll(Iterable[JsonPathNode])]]
    */
  @targetName("add")
  ...
  /** DSL for appending [[JsonPathNode]]s to this path.
    * Alias for [[appended(JsonPathNode)]].
    * @param node The [[JsonPathNode]] to append.
    * @return This path with the given node appended.
  ...
  /** Takes the first `n` nodes of this path.
    * @param n The number of nodes to take.
    * @return This path truncated to the first `n` [[JsonPathNode]]s.
    */
  def take(n: Int): JsonPath = copy(path = path.take(n))

  /** Drops the first `n` nodes of this path.
    * @param n The number of nodes to drop.
    * @return This path with the first `n` [[JsonPathNode]]s dropped.
    */
  def drop(n: Int): JsonPath = copy(path = path.drop(n))

  /** Takes the last `n` nodes of this path.
    * @param n The number of nodes to take.
    * @return This path truncated to the last `n` [[JsonPathNode]]s.
    */
  def takeRight(n: Int): JsonPath = copy(path = path.takeRight(n))

  /** Drops the last `n` nodes of this path.
    * @param n The number of nodes to drop.
    * @return This path with the last `n` [[JsonPathNode]]s dropped.
    */
  def dropRight(n: Int): JsonPath = copy(path = path.dropRight(n))

  /** Returns true if this path is empty and false otherwise. The root node of the path is not considered.
    * @return True if this path is empty, false otherwise.
    */
  def isEmpty: Boolean = path.isEmpty

  /** Returns true if this path is not empty and false otherwise. The root node of the path is not considered.
    * @return True if this path is not empty, false otherwise.
    */
  def nonEmpty: Boolean = path.nonEmpty

  /** Returns the number of [[JsonPathNode]] in this path.
    * @return The number of [[JsonPathNode]] in this path.
    */
  ...
  /** Returns the parent path of this path, or an empty path if this path is empty. For example, if this path is
    * `$.foo.bar`, this function would return `$.foo`.
    * @return The parent path of this path.
    */
  def parent: JsonPath = dropRight(1)

  /** Resolve the given [[JsonPath]] against this one. If the given path is not relative, then then the given path is simply returned. Otherwise, the given path is appended to this path.
    * For example, if this path is `$.foo` and the given path is `.bar[1,2,3]`, then `$.foo.bar[1,2,3]` is returned. If the given path is `$.deadbeef`, then `$.deadbeef` is returned regardless of whatever this path is.
    * @param that The path to resolve against this one.
  ...
  /** Resolve the given [[JsonPath]] against this one as a sibling. If the given path is not relative or if this path has no parent, then then the given path is simply returned. Otherwise, the given path is appended to this path's parent path.
    * For example, if this path is `$.foo.bar` and the given path is `.deadbeef[1,2,3]`, then `$.foo.deadbeef[1,2,3]` is returned. If the given path is `$.deadbeef`, then `$.deadbeef` is returned regardless of whatever this path is. And if this path is `$` then the given path is returned.
    * @param that The path to resolve as this one's sibling.
  ...
  /** Returns this path as an absolute path, IE, with a [[root]] of [[Current]].
    * @return This path as a dynamic path.
    */
...
object JsonPath {
  ...
  sealed trait JsonPathNode {
    ...
    /** Matches this [[JsonPathNode]] against the given [[Json]], returning all the values that match this node.
      * @param json The [[Json]] to match against.
      * @tparam Json The type of [[Json]].
      * @throws UnsupportedOperationException If the path contains a [[NonExecutableExpression]].
      * @return A list of all JSON values that match this node.
      */
    @throws[UnsupportedOperationException]
  ...
  /** Recursively descends through exposing all associative JSON nodes throughout the target value. If a selector is
    * provided, then that selector will be applied to each associative discovered by the recursive descent.
    *
    * @param selector
  ...
  sealed trait Selector {
    ...
    /** Applies this [[Selector]] to the given [[Json]] instance and returns an [[Iterable]] of all values that this selector applies to.
      * @param json The [[Json]] to match against.
      * @tparam Json The type of [[Json]].
      * @return An [[Iterable]] of all JSON values that this selector applies to.
      */
  ...
  /** Selects all direct children of an associative JSON value. If the target value is atomic, nothing is matched.
    */
  ...
  /** Selects the union of two or more [[SingleSelector]]s, ie, this [[Selector]] selects values which match any of the given [[SingleSelector]]s.
    * @param head The first [[SingleSelector]] in the union.
    * @param second The second [[SingleSelector]] in the union.
    * @param tail The remaining [[SingleSelector]]s in the union.
    */
  ...
  /** Selects a slice of a JSON array. If the target JSON is not an array, nothing is selected. At least one of `start`, `end`, and `step` must be specified.
    * @param start The start of the slice, inclusive. If negative, it is relative to the end of the array.
  ...
  object Slice {
    ...
    /** Creates a slice with the given start value, ie, the subarray from the given idnex.
    ...
    /** Creates a slice which takes the first `n` elements of the array.
      * Alias for [[end(Int)]].
  ...
  sealed trait Expression {
    ...
    /** Applies the expression to the given context.
      * @param root The root document of the [[JsonPath]] evaluation.
      * @param current The current document for this expression, ie, the json values matched by the [[JsonPathNode]] containing this expression.
      * @tparam Json The type of the json.
      * @throws UnsupportedOperationException If the expression is not executable.
